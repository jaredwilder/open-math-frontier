...
**Status:** PROVED IN SOURCE CAMPAIGN; exact novelty still under specialist audit
...
\[
\boxed{k e^{\Delta_k^2U}+r e^{\Delta_r^2U}=k+r.}
\]
...
If `W=U-V` had a strict interior maximum, then
...
and strict monotonicity of the exponential contradicts equality of the normalized equations.

The same argument applies to an interior minimum. Therefore

\[
\boxed{
\max_\Omega(U-V)\le\max_{\partial\Omega}(U-V),
}
\]
...
For `t<t_*`, `U(t)=log(D(t)/B)` is finite and satisfies the normalized elliptic equation.

Boundary positivity and compactness in time give a uniform finite lower bound for `U` on the boundary. The minimum principle gives the same lower bound in the interior.

But an interior determinant approaching zero would force `U -> -infinity`, contradicting that lower bound. Therefore no such first interior zero exists. QED.
...
Target-time boundary positivity alone is insufficient because `log(D/B)` ceases to be defined after an interior determinant becomes negative. The homotopy hypothesis is load-bearing.

## Novelty note

Discrete maximum principles and Desnanot–Jacobi/Toda-type recurrences are classical. A targeted search before this release did not locate this exact logarithmic normalization and boundary-homotopy positivity theorem for consecutive Toeplitz-minor arrays. This is therefore published as a surviving theorem with **novelty unresolved**, not as a historical-first claim.
