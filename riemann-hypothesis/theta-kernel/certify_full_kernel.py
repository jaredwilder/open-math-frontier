"""Resolution push for the full kernel, with the defect fixed and the work cut.

Two cost cuts that cost no rigor:
  (1) NMAX = 2.  The n = 3 term is bounded by exp(-9 pi) = 1.6e-13 of scale and goes into the
      remainder, which was already 15 orders below the quantities.
  (2) cache the exp brackets by argument -- the cells share endpoints, so each endpoint's
      exponentials are computed once.
  (3) only the three moments actually needed.
"""
...
for j in (0, 2, 4):
    ...
    for i in range(1, N + 1):
        ...
        for n in range(1, NMAX + 1):
            c = iadd(c, cell(a, b, j, n))
        tot = iadd(tot, (H * c[0], H * c[1]))
    sr = series_rem(j); tl = tail(j)
    lo = dn(tot[0] - sr - tl); hi = up(tot[1] + sr + tl)
    EN[j] = (lo, hi)
    m = (lo + hi) / 2
    print("   I_%d in [%.16f, %.16f]  rel width %.3e  (srem %.2e tail %.2e)"
          % (j, float(lo), float(hi), float((hi - lo) / m), float(sr), float(tl)), flush=True)

lhs = imul(EN[0], EN[4]); rhs = isc(F(3), imul(EN[2], EN[2]))
d = isub(rhs, lhs)
print(flush=True)
print("   3 I_2^2 - I_0 I_4 in [%.12e, %.12e]" % (float(d[0]), float(d[1])), flush=True)
if d[0] > 0:
    print("*** CERTIFIED FOR THE FULL KERNEL: >= %.10e > 0" % float(d[0]), flush=True)
else:
