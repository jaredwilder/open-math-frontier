"""K244: monotone cell bounds using the kernel-sealed unimodality, with a RATIONAL peak bracket.

The log-derivative j/x + c - a e^x is strictly decreasing, so each positive and negative
piece is unimodal with a single peak. Bracket the peak rationally and use monotone endpoint
bounds off the peak cells.
"""
...
def piece_val(x, j, cnum, n, positive):
    ...
    g = e_br(cnum * x)
    if positive:
        return (dn(x ** j * 2 * PI_LO * PI_LO * n4 * g[0] * d[0]),
                up(x ** j * 2 * PI_HI * PI_HI * n4 * g[1] * d[1]))
    return (dn(x ** j * 3 * PI_LO * n2 * g[0] * d[0]),
            up(x ** j * 3 * PI_HI * n2 * g[1] * d[1]))

def srem(j):
    t = F(0)
    for n in range(NMAX + 1, NMAX + 8):
        n2 = n * n; n4 = n2 * n2
        t = up(t + up(2 * PI_HI * PI_HI * n4 * e_br(F(9, 4) * X)[1]
...
def piece_cell(a, b, j, n, positive):
    ...
    if b < p[0] or a > p[1]:
        return (min(va[0], vb[0]), max(va[1], vb[1]))
...
for j in (0, 2, 4, 6, 8, 10):
    ...
    for i in range(1, N + 1):
        ...
        for n in range(1, NMAX + 1):
            c = iadd(c, isub(piece_cell(a, b, j, n, True), piece_cell(a, b, j, n, False)))
...
print(flush=True)
print("series remainder and truncation tail INCLUDED in every enclosure.", flush=True)
