"""THE LAST OPEN COMPUTATION on the route's lower half: the higher-term tail, in exact
rational interval arithmetic.

Claim to certify:  for all u >= 0, with E = e^{4u} >= 1,
    | sum_{n>=2} f_n(u) |  <=  2 pi^2 e^{9u} * sum_{n>=2} n^4 e^{-pi n^2 E}
and the sign argument K423 needs only that this tail does not flip the sign of
(log Phi)'' - 4 (log Phi)'.  Certify the RELATIVE tail bound at its worst point u = 0
in exact rationals: tail / Phi(0) <= a certified rational.
"""
...
e9lo,_=exp_br(F(0)); ep_lo,ep_hi=exp_neg_br(PI_HI)
...
print()
print("   is the certified relative tail below 1/100 ?  %s"%(rel < F(1,100)),flush=True)
